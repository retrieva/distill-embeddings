...
def main(args):
    ...
    # 統計値とか、前処理の結果だけ拝借する
    triplet_base_dir = "data/triplet-eng"
    checkpoint_dir = output_path / "checkpoints"
    with open(Path(triplet_base_dir) / "dataset_summary.json") as f:
    ...
    subset_to_target_num_examples = {
        subset: int(min(num_examples, 1_000_000) * down_sampling_ratio)
        for subset, num_examples in subset_to_num_examples.items()
    }
    print(subset_to_target_num_examples)
    with open(output_path / "dataset_summary.json", "w") as f:
        json.dump(subset_to_target_num_examples, f, indent=4)
    encode_datasets = []
    if args.w_instruction:
        with open(Path(args.output_dir) / "instruction.json") as f:
            instruction = json.load(f)
    else:
        instruction = dict.fromkeys(dataset_summary.keys(), "")
    for subset in subset_to_target_num_examples.keys():
        dataset = pd.read_json(f"{triplet_base_dir}/{subset}.jsonl", orient="records", lines=True)
    ...
    # データサイズの整合性をチェック
    assert len(long_texts) == len(long_teacher_features), (
        f"Long texts size mismatch: {len(long_texts)} vs {len(long_teacher_features)}"
    )
    assert len(short_texts) == len(short_teacher_features), (
        f"Short texts size mismatch: {len(short_texts)} vs {len(short_teacher_features)}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        # "--teacher_model", type=str, default="Qwen/Qwen3-Embedding-4B", help="Path to the teacher model"
    )
