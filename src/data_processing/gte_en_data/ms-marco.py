...
def flatten_passages_batched(examples):
    ...
    for i in range(len(examples["query"])):
        ...
        for passage_text, is_selected in zip(
            examples["passages"][i]["passage_text"],
            examples["passages"][i]["is_selected"]
        ):
            # 新しいリストに各情報を追加していく
...
pair_dataset = pair_dataset.rename_columns({"query":"anc", "passage_text":"pos"}).remove_columns(["query_id","answers","is_selected"])
