...
def main(args):
    ...
    if args.model_name.endswith(".ckpt"):
        ...
        model_weights = {k.removeprefix("student_model."): v 
    ...
    dataset = dataset.map(lambda x: {"anchor": x["anc"].strip(), 
                                     "positive": x["pos"][0].strip(),
                                     "negative": x["neg"][0].strip()})
    ...
    evaluation = mteb.MTEB(
        tasks=["AmazonCounterfactualClassification", "JSICK", "JSTS"], 
        task_langs=["jpn-Jpan"]
    )
