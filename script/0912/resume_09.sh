...
# Experiment params (match final_result/09.sh)
...
DISTILL_WEIGHT="0.98"
CHUNK_PARTS=4
MAX_EFF_PAIRS_PER_RANK=64
MAX_NEG_PER_SAMPLE=1
...
# Default to DeepSpeed to leverage 2 GPUs; train.py will safely disable it
# when resuming from a single-file .ckpt.
...
echo "Resuming 09 (kld, w-neg) from: ${CKPT} (run_id=${RUN_ID:-none})"
uv run python -m src.training.train \
  ...
  --max_effective_pairs_per_rank "${MAX_EFF_PAIRS_PER_RANK}" \
  --max_neg_per_sample "${MAX_NEG_PER_SAMPLE}" \
  ${STRATEGY_ARGS} \
  $( [ "${USE_POS}" = "true" ] && printf %s "--use_pos" ) \
  $( [ "${USE_NEG}" = "true" ] && printf %s "--use_neg" ) \
